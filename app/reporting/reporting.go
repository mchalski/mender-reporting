...
func (app *app) Reindex(ctx context.Context, tenantID, devID string, service string) error {
	...
	l.Debugf("reindexing for device %v:%v", tenantID, devID)
	...
	l.Debug("getting inventory device(s)")
	devs, err := app.invClient.GetDevices(ctx, tenantID, []string{devID})
	if err != nil {
		return err
	}

	l.Debugf("got inventory device(s) %v", devs)
	...
	if esdev == nil {
		...
		l.Debugf("stored new device %#+v", newdev)
		return nil
	}
	...
	l.Debugf("will update stored device")
